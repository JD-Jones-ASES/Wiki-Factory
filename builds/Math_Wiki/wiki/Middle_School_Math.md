---
title: "Middle School Math"
type: overview
aliases: ["Middle School", "Grades 6-8", "Pre-Algebra Hub"]
tags: ["#navigation", "#branch-pre-algebra"]
created: 2026-04-11
updated: 2026-04-11
source_refs: []
related: []
status: complete
confidence: high
summary: "Course hub for Middle School Math (grades 6-8). Whole numbers, integers, fractions, decimals, percents, ratios, basic geometry, and first algebra."
---

> [[_overview|Home]] > Middle School Math

# Middle School Math

**Grades 6-8. The foundations that everything else is built on.**

This is where arithmetic starts to become algebra. You will work with integers and the number line, learn how fractions and decimals fit together, master ratios and percents, meet variables and expressions for the first time, and pick up the geometry and data-analysis habits that high school math assumes you already have.

---

## What You'll Learn

- **Numbers and operations** --- integers, fractions, decimals, and how to compute with all three cleanly.
- **Ratios, rates, percents** --- the bridge from arithmetic to algebra.
- **Expressions and variables** --- reading, writing, and evaluating algebraic expressions.
- **Coordinate plane and inequalities** --- plotting points, graphing simple lines, writing and solving one- and two-step inequalities.
- **Geometry foundations** --- perimeter, area, volume, the Pythagorean theorem, angle relationships, similar triangles.
- **Data and probability** --- mean, median, mode, range, histograms, and simple probability.

---

## Suggested Learning Path

A reasonable order to read the lessons if you are new to middle-school math:

1. [[Place_Value_Rounding_And_Estimation|Place Value, Rounding, and Estimation]]
2. [[Integers_And_The_Number_Line|Integers and the Number Line]] → [[Adding_And_Subtracting_Integers|Adding/Subtracting Integers]] → [[Multiplying_And_Dividing_Integers|Multiplying/Dividing Integers]]
3. [[Order_Of_Operations|Order of Operations]]
4. [[Equivalent_Fractions_And_Simplifying|Equivalent Fractions]] → [[Adding_And_Subtracting_Fractions|Adding/Subtracting Fractions]] → [[Multiplying_Fractions|Multiplying]] → [[Dividing_Fractions|Dividing]]
5. [[Decimal_Place_Value_And_Comparing_Decimals|Decimal Place Value]] → [[Adding_And_Subtracting_Decimals|+/-]] → [[Multiplying_Decimals|×]] → [[Dividing_Decimals|÷]]
6. [[Ratios_And_Equivalent_Ratios|Ratios]] → [[Unit_Rates|Unit Rates]] → [[Proportions_And_Cross_Multiplication|Proportions]]
7. [[Finding_A_Percent_Of_A_Number|Percents]] → [[Percent_Increase_And_Decrease|Percent Change]]
8. [[Variables_And_Algebraic_Expressions|Variables and Expressions]] → [[The_Distributive_Property|Distributive Property]] → [[Evaluating_Expressions|Evaluating Expressions]]
9. [[Plotting_Points_And_The_Coordinate_Plane|Coordinate Plane]]
10. [[Writing_And_Graphing_Inequalities|Inequalities]] → [[Solving_One_Step_And_Two_Step_Inequalities|Solving Inequalities]]
11. [[Square_Roots_And_Cube_Roots|Square and Cube Roots]] → [[The_Pythagorean_Theorem|Pythagorean Theorem]]
12. [[Mean_Median_Mode_And_Range|Mean, Median, Mode, Range]] → [[Data_Displays|Data Displays]] → [[Probability_Of_Simple_And_Compound_Events|Probability]]

From here you are ready for [[Algebra_1|Algebra 1]].

---

## How to Practice

Every live topic below has a full lesson, 2+ worked examples, and a **Problems Involving {Topic}** widget at the bottom. Pick a problem type, pick a difficulty, choose how many problems you want, and click **Add to Vault**. Your saved problems live in your browser. Open [[Vault|🎒 Your Practice Vault]] whenever you are ready to review or download a worksheet.

---

<!-- AUTO:TOPICS:BEGIN -->

### Middle School Math topics --- 73 live / 73 total

**🟢 Live topics with practice widgets (73)**

- 🟢 [[Absolute_Value_And_Opposites|Absolute Value and Opposites]]
- 🟢 [[Adding_And_Subtracting_Decimals|Adding and Subtracting Decimals]]
- 🟢 [[Adding_And_Subtracting_Fractions|Adding and Subtracting Fractions]]
- 🟢 [[Adding_And_Subtracting_Integers|Adding and Subtracting Integers]]
- 🟢 [[Applications_Of_Proportional_Reasoning|Applications of Proportional Reasoning]]
- 🟢 [[Applications_Of_The_Pythagorean_Theorem|Applications of the Pythagorean Theorem]]
- 🟢 [[Applications_Tax_Tip_Discount_And_Simple_Interest|Applications: Tax, Tip, Discount, and Simple Interest]]
- 🟢 [[Arithmetic_Sequences_And_Linear_Patterns|Arithmetic Sequences and Linear Patterns]]
- 🟢 [[Circumference_And_Area_Of_Circles|Circumference and Area of Circles]]
- 🟢 [[Classifying_Triangles_And_Quadrilaterals|Classifying Triangles and Quadrilaterals]]
- 🟢 [[Comparing_And_Ordering_Fractions|Comparing and Ordering Fractions]]
- 🟢 [[Composite_Figures|Composite Figures]]
- 🟢 [[Data_Displays|Data Displays]]
- 🟢 [[Data_Displays_And_Measures_Of_Spread|Data Displays and Measures of Spread]]
- 🟢 [[Decimal_Place_Value_And_Comparing_Decimals|Decimal Place Value and Comparing Decimals]]
- 🟢 [[Dividing_Decimals|Dividing Decimals]]
- 🟢 [[Dividing_Fractions|Dividing Fractions]]
- 🟢 [[Divisibility_Factors_And_Prime_Factorization|Divisibility, Factors, and Prime Factorization]]
- 🟢 [[Equivalent_Fractions_And_Simplifying|Equivalent Fractions and Simplifying]]
- 🟢 [[Evaluating_Expressions|Evaluating Expressions]]
- 🟢 [[Exponents_And_Powers|Exponents and Powers]]
- 🟢 [[Finding_A_Percent_Of_A_Number|Finding a Percent of a Number]]
- 🟢 [[Fractions_Decimals_And_Percents|Fractions, Decimals, and Percents]]
- 🟢 [[Graphing_Linear_Equations_From_Tables|Graphing Linear Equations from Tables]]
- 🟢 [[Greatest_Common_Factor_And_Least_Common_Multiple|Greatest Common Factor and Least Common Multiple]]
- 🟢 [[Integers_And_The_Number_Line|Integers and the Number Line]]
- 🟢 [[Irrational_Numbers_And_Real_Numbers|Irrational Numbers and Real Numbers]]
- 🟢 [[Mean_Median_Mode_And_Range|Mean, Median, Mode, and Range]]
- 🟢 [[Mixed_Numbers_And_Improper_Fractions|Mixed Numbers and Improper Fractions]]
- 🟢 [[Multiplying_Decimals|Multiplying Decimals]]
- 🟢 [[Multiplying_Fractions|Multiplying Fractions]]
- 🟢 [[Multiplying_And_Dividing_Integers|Multiplying and Dividing Integers]]
- 🟢 [[Order_Of_Operations|Order of Operations]]
- 🟢 [[Percent_Increase_And_Decrease|Percent Increase and Decrease]]
- 🟢 [[Perimeter_And_Area_Of_Polygons|Perimeter and Area of Polygons]]
- 🟢 [[Place_Value_Rounding_And_Estimation|Place Value, Rounding, and Estimation]]
- 🟢 [[Plotting_Points_And_The_Coordinate_Plane|Plotting Points and the Coordinate Plane]]
- 🟢 [[Points_Lines_Angles_And_Angle_Relationships|Points, Lines, Angles, and Angle Relationships]]
- 🟢 [[Probability_Of_Simple_And_Compound_Events|Probability of Simple and Compound Events]]
- 🟢 [[Product_Power_And_Quotient_Rules|Product Power and Quotient Rules]]
- 🟢 [[Properties_Of_Addition_And_Multiplication|Properties of Addition and Multiplication]]
- 🟢 [[Proportions_And_Cross_Multiplication|Proportions and Cross Multiplication]]
- 🟢 [[Rational_Exponents|Rational Exponents]]
- 🟢 [[Ratios_And_Equivalent_Ratios|Ratios and Equivalent Ratios]]
- 🟢 [[Ratios_Rates_And_Proportions|Ratios, Rates, and Proportions]]
- 🟢 [[Repeating_Decimals_As_Fractions|Repeating Decimals as Fractions]]
- 🟢 [[Scale_Drawings_And_Maps|Scale Drawings and Maps]]
- 🟢 [[Similar_Triangles|Similar Triangles]]
- 🟢 [[Simple_And_Compound_Interest|Simple and Compound Interest]]
- 🟢 [[Simplifying_Expressions|Simplifying Expressions]]
- 🟢 [[Slope_Intercept_Form|Slope-Intercept Form]]
- 🟢 [[Solving_One_Step_Equations_Addition_And_Subtraction|Solving One-Step Equations (Addition and Subtraction)]]
- 🟢 [[Solving_One_Step_Equations_Multiplication_And_Division|Solving One-Step Equations (Multiplication and Division)]]
- 🟢 [[Solving_One_Step_And_Two_Step_Inequalities|Solving One-Step and Two-Step Inequalities]]
- 🟢 [[Solving_Two_Step_Equations|Solving Two-Step Equations]]
- 🟢 [[Square_Roots_And_Cube_Roots|Square Roots and Cube Roots]]
- 🟢 [[Surface_Area_And_Volume_Of_Spheres|Surface Area and Volume of Spheres]]
- 🟢 [[Surface_Area_Of_Prisms_And_Cylinders|Surface Area of Prisms and Cylinders]]
- 🟢 [[The_Distance_Formula|The Distance Formula]]
- 🟢 [[The_Distributive_Property|The Distributive Property]]
- 🟢 [[The_Distributive_Property_With_Variables|The Distributive Property with Variables]]
- 🟢 [[The_Midpoint_Formula|The Midpoint Formula]]
- 🟢 [[The_Percent_Equation|The Percent Equation]]
- 🟢 [[The_Pythagorean_Theorem|The Pythagorean Theorem]]
- 🟢 [[Translating_Words_To_Algebraic_Expressions|Translating Words to Algebraic Expressions]]
- 🟢 [[Triangle_Angle_Sum_And_Exterior_Angles|Triangle Angle Sum and Exterior Angles]]
- 🟢 [[Understanding_Percents|Understanding Percents]]
- 🟢 [[Unit_Rates|Unit Rates]]
- 🟢 [[Units_Of_Measurement_And_Conversion|Units of Measurement and Conversion]]
- 🟢 [[Variables_And_Algebraic_Expressions|Variables and Algebraic Expressions]]
- 🟢 [[Volume_Of_Prisms_And_Cylinders|Volume of Prisms and Cylinders]]
- 🟢 [[Volume_Of_Pyramids_And_Cones|Volume of Pyramids and Cones]]
- 🟢 [[Writing_And_Graphing_Inequalities|Writing and Graphing Inequalities]]

<!-- AUTO:TOPICS:END -->

---

## See Also

- [[Algebra_1|📗 Algebra 1]] --- the next course in the sequence
- [[Geometry|📕 Geometry]] --- the HS Geometry course
- [[Topics_Overview|📖 All Topics]] --- alphabetical index
- [[Vault|🎒 Your Practice Vault]]
- [[Topic_Status|📊 Progress Dashboard]]
- [[_overview|Home]]
